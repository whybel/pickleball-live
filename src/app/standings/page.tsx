      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
        {Object.keys(groups).sort().map((group) => (
          <div key={group} style={{ background: '#111111', border: '1px solid #1a1a1a', borderRadius: '4px', overflow: 'hidden' }}>
            <div style={{ background: 'rgba(201, 169, 89, 0.1)', padding: '16px 20px', borderBottom: '1px solid #1a1a1a' }}>
              <h2 style={{ fontSize: '16px', fontWeight: 'bold', color: '#C9A959', margin: 0, textTransform: 'uppercase' }}>
                Group {group}
              </h2>
            </div>
            <div style={{ padding: '0' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 60px 60px 60px', fontSize: '11px', color: '#888888', textTransform: 'uppercase', letterSpacing: '1px', padding: '12px 20px', borderBottom: '1px solid #1a1a1a', background: '#0a0a0a' }}>
                <div>Team</div>
              ...
              {groups[group].map((team: any, index: number) => (
                <div key={team.id} style={{ display: 'grid', gridTemplateColumns: '1fr 60px 60px 60px', fontSize: '13px', padding: '12px 20px', borderBottom: '1px solid #1a1a1a', alignItems: 'center' }}>
                  <div style={{ fontWeight: '500', color: '#ffffff' }}>{team.team?.name}</div>
                  <div style={{ textAlign: 'center', color: '#ffffff' }}>{team.wins || 0}</div>
                  <div style={{ textAlign: 'center', color: '#ffffff' }}>{team.losses || 0}</div>
                  <div style={{ textAlign: 'center' }}>
                    <span style={{ background: '#1a1a1a', color: '#C9A959', fontSize: '11px', fontWeight: '600', padding: '4px 8px', borderRadius: '4px', minWidth: '32px', display: 'inline-block' }}>
                      {team.rank || index + 1}
                    </span>
                  </div>
                </div>
